...
void MainWindow::plotFunction()
{
	...
	windowTwo.DrawOptionTwo(MValue, KValue, NValue);
	windowTwo.setSize(550,550);
}
...
//Create a button that creates a function graph	
	dock = new QDockWidget(this);
    dock->setAllowedAreas(Qt::LeftDockWidgetArea);	
	
	QPushButton * GraphButton = new QPushButton("Create Graph", dock);
	dock->setWidget(GraphButton);
	addDockWidget(Qt::LeftDockWidgetArea, dock);
	
//connect graph button to custom slot

	QObject::connect(GraphButton, SIGNAL(pressed()),
	 SLOT(plotFunction()));	
...
//do some calculating
	...
	KValue = KSpinBox->value();
	NValue =	NSpinBox->value();
	MValue = MemorySpinBox->value();
	double R_Word = log2(MValue);
