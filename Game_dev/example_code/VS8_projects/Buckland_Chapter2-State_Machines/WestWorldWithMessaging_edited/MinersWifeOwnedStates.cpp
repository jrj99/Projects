...
bool CookStew::OnMessage(MinersWife* wife, const Telegram& msg)
{
  ...
  switch(msg.Msg)
  {
    case Msg_StewReady:
    {
	  ...
	  if (wife->PantryEmpty() == true ) {
		  wife->GetFSM()->ChangeState(GoShopping::Instance());
	  }
	  else {
...
//--------------------The following Grocer Code has been added by Jared Jones ---------------------------------------------

GoShopping* GoShopping::Instance()
{
	static GoShopping instance;

	return &instance;
}

void GoShopping::Enter(MinersWife* wife)
{
	//if Elsa is not already located at the grocery store, she must
	//change location to the store.
	if (wife->Location() != generalstore) {

		cout << "\n " << GetNameOfEntity(wife->ID()) << ":  " << "We're out of food!"
			<< " S' pose I should head over to th'a general store.";
	
		wife->ChangeLocation(generalstore);

		//Send a delayed message to grocer to trigger Elsa - specific dialouge.
		//time delay simulates the amount of time needed for elsa to walk
		//to the general store.

		Dispatch->DispatchMessage(0.5,                  //time delay
			wife->ID(),           //sender ID
			ent_Woolsworth,           //receiver ID
			Msg_GroceryShopping,        //msg
			NO_ADDITIONAL_INFO);
	}

}
void GoShopping::Execute(MinersWife* wife)
{

}
void GoShopping::Exit(MinersWife* wife)
{
	SetTextColor(FOREGROUND_GREEN | FOREGROUND_INTENSITY);

	wife->RestockPantry();

	cout << "\n " << GetNameOfEntity(wife->ID()) << ":  " << "Restocked tha' Pantry.";


}

bool GoShopping::OnMessage(MinersWife* wife, const Telegram& msg)
{
	SetTextColor(BACKGROUND_RED | FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
	...
	case Msg_GroceriesRecieved:
	{
		...
		cout << "\n" << GetNameOfEntity(wife->ID()) <<
			": Got my groceries in hand. I best head to tha' homestead";
